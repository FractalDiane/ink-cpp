...
void InkStoryTracking::update_visit_count_variables(const std::vector<KnotStatus>& knots_stack, Stitch* current_stitch, ExpressionParserV2::StoryVariableInfo& story_variable_info) {
	...
	for (auto& knot : knot_stats) {
		...
		for (auto it = knots_stack.rbegin(); it != knots_stack.rend(); ++it) {
			if (it->knot->uuid == knot.first) {
				//story_variable_info.constants[stitch.name] = TK(stitch.times_visited);
				//break;
				containing_knot = it->knot;
				break;
			}
		}
		...
		for (const Stitch& stitch_id : containing_knot->stitches) {
			StitchStats& stitch = stitch_stats[stitch_id.uuid];
			std::string stitch_name = std::format("{}.{}", containing_knot->name, stitch_id.name);
			story_variable_info.constants[stitch_name] = TK(stitch.times_visited);
			...
			for (const GatherPoint& gather_id : stitch_id.gather_points) {
				SubKnotStats& gather_point = gather_point_stats[gather_id.uuid];
				std::string gather_point_name = std::format("{}.{}.{}", containing_knot->name, stitch_id.name, gather_id.name);
				story_variable_info.constants[gather_point_name] = TK(gather_point.times_visited);

				if (containing_knot) {
					std::string gather_point_name_2 = std::format("{}.{}", stitch_id.name, gather_id.name);
					story_variable_info.constants[gather_point_name_2] = TK(gather_point.times_visited);
				}

				if (current_stitch && current_stitch->uuid == stitch_id.uuid) {
					story_variable_info.constants[gather_point.name] = TK(gather_point.times_visited);
				}
			}
		}

		for (const GatherPoint& gather_id : containing_knot->gather_points) {
			SubKnotStats& gather_point = gather_point_stats[gather_id.uuid];
			std::string gather_point_name = std::format("{}.{}", containing_knot->name, gather_id.name);
			story_variable_info.constants[gather_point_name] = TK(gather_point.times_visited);
